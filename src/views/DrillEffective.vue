...
export default {
  ...
  methods: {
    ...
    buildTable() {
      ...
      if (b) {
          ...
          b.header.forEach((h) => {
            if (this.idx[h]) {
              hr = [
                ...hr,
                {
                  text: this.$t("DrillReport.impact-" + h),
                  value: h,
                },
              ];
              t = [...t, this.$t("DrillReport.impact-" + h)];
            }
          });
          cv = [...cv, t];
        }
        if (b.detail) {
          ...
          dt.forEach((d) => {
            if (d.userName == 0) {
              d.userName = this.$t("DrillReport.impact-average");
            }
            // d.userName = this.$t("Overview.status" + d.status);
            t = [];
            b.header.forEach((h) => {
              if (this.idx[h]) {
                t = [...t, d[h]];
    ...
    toCSV() {
      ...
      x.setAttribute(
        "download",
        this.paramdata.package_exp.package[this.signedinuser.companyName] +
          "_" +
          this.selectedDrill +
          ".csv"
      );
