    ...
    checkMessage(ev) {
      ...
      if (ev.action === "COMPLETED") {
        ...
        var tx =
          ...
          ev.content.valueToComplete +this.$t('DrillPlatform.valueDollar')+this.$t('DrillPlatform.value')+
          ", " + this.$t('DrillPlatform.valueUnit')+ ev.content.orderValue+
          "<br /><br />"+ this.$t('DrillPlatform.daysTarget')+this.$t('DrillPlatform.week')+ev.content.control.numberOfDays/5+this.$t('DrillPlatform.week1')+", "+
          this.$t("DrillPlatform.total")+this.$t('DrillPlatform.day')+
          ev.content.control.numberOfDays +this.$t('DrillPlatform.day1')+
          "<br /><br />" +
        ...
        // );
        // if (ev.content.control.valueToComplete) {
        //   this.buildMessage(
        //     this.$t("DrillPlatform.yourBoss"),
        //     this.$t("DrillPlatform.valueTarget") +
        //       ev.content.control.valueToComplete
        //   );
        // }
        // this.buildMessage(
        //   this.$t("DrillPlatform.yourBoss"),
        //   this.$t("DrillPlatform.daysTarget") + ev.content.control.numberOfDays
        // );
        // this.buildMessage(
        //   this.$t("DrillPlatform.yourBoss"),
        //   this.$t("DrillPlatform.timeTarget") +
      ...
      if (ev.action === "MORE_ACTIONS" || ev.action === "SELECT_STYLE") {
          ...
          if (stm.count) {
            ...
            if (c < stm.count) {
              ...
              if (stm.description && stm.description[c].length > 0) {
                this.buildMessage(
                  this.$t("DrillPlatform.notice"),
                  stm.staffExp[c] + " " + this.$t(stm.description[c]),
                  null,
                  -1
                );
                stm.description[c] = "";
        ...
        evn = ev.content.day[ev.dayNumber]["event"];
        if (evn.count) {
          let c = 0;
          if (c < evn.count) {
...
//      await this.readParam()
    ...
    flashMemberAdded() {
      if (
        ...
        this.flashTime = setTimeout(this.clearMemberAdded, 5000);
      }
    },
    clearMemberAdded() {
    ...
    profileChanged() {
      ...
      for (let c = 1; c <= this.drillRecord.content.control.maxMembers; c++) {
        if (this.drillRecord.content.members[c].addedAvg !== 0) {
          v++;
        }
      }
      return v;
